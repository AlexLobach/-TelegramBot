...
async def welcome(message: types.Message):
    ...
    await bot.send_message(
        ...
        message.chat.id,
        # указываем отправляемое сообщение hello + имя пользователя + текст
...
async def contacts(message: types.Message):
    # если переданное боту сообщение = 'Контакты'
    if message.text == 'Контакты':
        ...
        await bot.send_message(message.chat.id,
                               reply_markup=keyboard.user_id, parse_mode="Markdown",
                               text='*Хотете посмортеть свой ID?*.\nЕсли нет, нажмите кнопку *"вернуться обратно"*')

    elif message.text == "Отправить фото":

        await bot.send_photo(message.chat.id, open('download.jpg', 'rb'))
        await asyncio.sleep(0.4)


"""_______________________________________ обработка inline-кнопки "ID"______________________________________________"""


@dp.callback_query_handler(text_contains="button_id")
async def id_button(call: types.CallbackQuery):
    await bot.send_message(call.message.chat.id,
                           text=f'Ваш ID: *{call.message.chat.id}*',
                           parse_mode="Markdown")


"""_______________________________ обработка inline-кнопки "Вернуться назад"_________________________________________"""


@dp.callback_query_handler(text_contains="back_again")
